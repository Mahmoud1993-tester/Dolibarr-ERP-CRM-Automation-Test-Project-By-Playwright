...
test ('Leave login and password empty', async({page})=> {
  ...
  await page.fill("#login", '') // Login Name
  ...
  await page.fill("//input[@name='email']", "hedox23547@bamsrad.com");   // E-mail
  ...
  // Asseration Messsage Invalid Input
  await expect(page.locator('.jnotify-message')).toBeVisible();
  await expect(page.locator('.jnotify-message')).toHaveText('Login is not defined.');
